...
with st.sidebar:
    ...
    # Stock selection (dropdown with optional custom input)
    st.subheader("Stock Selection")
    ticker_options = list(settings.TICKERS) if hasattr(settings, 'TICKERS') else [settings.TICKER]
    if settings.TICKER not in ticker_options:
        ticker_options.insert(0, settings.TICKER)
    ticker_options = ticker_options + ["Other"]
    selected = st.selectbox("Choose Ticker", options=ticker_options, index=0,
                            help="Select a ticker or choose 'Other' to type a custom symbol")
    if selected == "Other":
        ticker = st.text_input("Enter Stock Ticker", value=settings.TICKER, help="e.g., AAPL, GOOGL, MSFT, TSLA")
    else:
        ticker = selected
    ...
    if model_info:
        ...
        # Display model information and metrics directly (no expander)
        ...
        st.info(f"Click **'Auto-Train Model'** below to automatically fetch data and train a model for {ticker}.")
    ...
    if not model_path or not os.path.exists(model_path):
        ...
        if st.button("Auto-Train Model", use_container_width=True, type="primary"):
            # Session state to track training
...
if 'auto_train' in st.session_state and st.session_state.auto_train:
    train_ticker = st.session_state.train_ticker
    
    # Create a placeholder for the progress container
    progress_container = st.container()
    
    with progress_container:
        st.markdown("---")
        st.header("⏳ Auto-Training in Progress")
        ...
        try:
            # Step 1: Fetch data
            status_text.text("📡 Step 1/3: Fetching stock data from Yahoo Finance...")
            progress_bar.progress(20)
            logs.append("✓ Fetching data...")
            log_container.text_area("Training Log", "\n".join(logs), height=150, disabled=True)
            
            ingestor = DataIngestor(ticker=train_ticker)
            ingestor.fetch_and_save()
            
            logs.append(f"✓ Data fetched successfully for {train_ticker}")
            log_container.text_area("Training Log", "\n".join(logs), height=150, disabled=True)
            
            # Step 2: Train model
            status_text.text("🤖 Step 2/3: Training LSTM model (this may take a few minutes)...")
            progress_bar.progress(50)
            logs.append("✓ Starting model training...")
            log_container.text_area("Training Log", "\n".join(logs), height=150, disabled=True)
            
            # Import training function
            from src.model_trainer import ModelTrainer
            
            trainer = ModelTrainer(ticker=train_ticker)
            model, history, metrics, scaler = trainer.train()
            ...
            **Timestamp:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
            ...
            logs.append("✓ All steps completed!")
            log_container.text_area("Training Log", "\n".join(logs), height=150, disabled=True)
            ...
            **Troubleshooting:**
            - Ensure the ticker symbol is valid (e.g., TSLA, AAPL, GOOGL)
...
# Main content - only show if we're not training
elif 'auto_train' not in st.session_state or not st.session_state.auto_train:
    ...
    if model_path and os.path.exists(model_path):
        ...
        if df is not None and len(df) > lookback:
            ...
            with st.expander("ℹ️ About This Dashboard"):
                ...
                **Features:**
                ...
                - **Auto-training** for new stocks (data fetch + model training)
        ...
        st.info("Click **'Auto-Train Model'** in the sidebar to automatically fetch data and train a model.")
